  ...
  return (
    ...
    >
      {selectedSlots.length > 1 ? (
        <TabsList
          className="grid h-auto w-full gap-px rounded-lg bg-border/70 p-px"
          ...
          {selectedSlots.map((slot) => {
            ...
            return (
              <TabsTrigger
                key={slot}
                value={slot}
                className="h-9 rounded-md bg-background px-2 text-xs data-[state=active]:shadow-none"
              >
                {label}
              </TabsTrigger>
            );
          ...
          <TabsContent key={slot} value={slot} className="mt-0">
            <div className="space-y-3 rounded-lg border border-border/70 bg-background/70 p-3">
              ...
              <div className="space-y-2">
                ...
                <textarea
                  className="min-h-28 w-full rounded-md border border-input bg-background/75 px-3 py-2.5 text-sm leading-6 outline-none ring-offset-background placeholder:text-muted-foreground focus-visible:ring-2 focus-visible:ring-ring"
                  value={entry?.prompt ?? ""}
              ...
              <div className="space-y-2">
                ...
                <textarea
                  className="min-h-20 w-full rounded-md border border-input bg-background/75 px-3 py-2.5 text-sm leading-6 outline-none ring-offset-background placeholder:text-muted-foreground focus-visible:ring-2 focus-visible:ring-ring"
